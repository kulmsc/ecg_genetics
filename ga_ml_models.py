...
class Models():
        ...
        with open(fname) as f:
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    newline = [a if a != 'NA' else 'nan' for a in line.rstrip().split()]
                    rows.append(np.array(newline, dtype = "float"))
    ...
    def load_genetic_data(self, genetic_path, pheno_path, inc_stat = True):
        if os.path.isfile('genetic_data.pl'):
            all_genetic = pickle.load(open("genetic_data.pl",'rb'))
        else:
            all_genetic = self.load_big_file(genetic_path)
            pickle.dump(all_genetic, open("genetic_data.pl",'wb'), protocol=4)
        ...
        suffix = '.'.join(genetic_path.split('/')[8].split('.')[1:6])
        self.genetic_stats = np.loadtxt('/'+mainPath+"/afibStats."+suffix)                     #from afibSS, same order as bim
        self.genetic_alleles = np.loadtxt('/'+mainPath+"/afibAlleles."+suffix, dtype = 'str')  #from afibSS, same order as bim
        genetic_ref = np.loadtxt('/'+mainPath+"/afibRef."+suffix, dtype = 'str')               #from the imputed qc
        genetic_rsids = np.loadtxt('/'+mainPath+"/afibRsids."+suffix, dtype = 'str')           #from afibSS, same order as bim
        ...
        for i in range(genetic_ref.shape[0]):
            ...
            base_match = {'A':'T', 'T':'A', 'C':'G', 'G':'C'}
            base_act = "bad"            

            if self.genetic_alleles[i,0].upper() == genetic_ref[refIndex,1]:
                if self.genetic_alleles[i,1].upper() == genetic_ref[refIndex,2]:
                    base_act = "nothing"
                elif base_match[self.genetic_alleles[i,1].upper()] == genetic_ref[refIndex,2]:
                    base_act = "nothing"
            elif base_match[self.genetic_alleles[i,0].upper()] == genetic_ref[refIndex,1]:
                if self.genetic_alleles[i,1].upper() == genetic_ref[refIndex,2]:
                    base_act = "sub"
                elif base_match[self.genetic_alleles[i,1].upper()] == genetic_ref[refIndex,2]:
                    base_act = "sub"
            elif self.genetic_alleles[i,0].upper() == genetic_ref[refIndex,2]:
                if self.genetic_alleles[i,1].upper() == genetic_ref[refIndex,1]:
                    base_act = "swapstat"
                elif base_match[self.genetic_alleles[i,1].upper()] == genetic_ref[refIndex,1]:
                    base_act = "swapstat"
            elif base_match[self.genetic_alleles[i,0].upper()] == genetic_ref[refIndex,2]:
                if self.genetic_alleles[i,1].upper() == genetic_ref[refIndex,1]:
                    base_act = "both"
                elif base_match[self.genetic_alleles[i,1].upper()] == genetic_ref[refIndex,1]:
                    base_act = "both"
            ...
            elif base_act == "nothing":
                pass
            elif base_act == "swapstat" or base_act == "both":
                self.genetic_stats[i,1] = -self.genetic_stats[i,1]
            elif base_act == "sub 0" or base_act == "both":
                self.afib_data_genetic[i,:] = 2 - self.afib_data_genetic[i,:]
                self.normal_training_genetic[i,:] = 2 - self.normal_training_genetic[i,:]
                self.normal_testing_genetic[i,:] = 2 - self.normal_testing_genetic[i,:]
            ...
            #    bad_collector.append(i)
            #elif self.genetic_alleles[i,0].upper() == 'A' and genetic_ref[refIndex,1] == 'T':
            #else:
            #    print("Bad")
            #    pdb.set_trace()
    ...
    def get_genetic_nn(self):
        input_shape = self.afib_data_genetic.shape
        # 'lr', 'l1', 'l2', 'l3', 'd1', 'd2', 'd3', 'act', 'optim'

        #dense
        model = Sequential()
        model.add(Dense(self.opts['l1'], input_dim = input_shape[1], activation = self.opts['act']))
        model.add(Dropout(self.opts['d1']))
        if self.opts['l2'] != 0:
            model.add(Dense(self.opts['l2'], activation = self.opts['act']))
            if self.opts['d2']:
                model.add(Dropout(0.3))
            if self.opts['l3'] != 0:
                model.add(Dense(self.opts['l3'], activation = self.opts['act']))
                if self.opts['d3']:
                    model.add(Dropout(0.3))
        ...
        if self.opts['optim'] == 'adam':
            my_optim = keras.optimizers.Adam(lr = self.opts['lr'])
        elif self.opts['optim'] == 'adamax':
            my_optim = keras.optimizers.Adamax(lr = self.opts['lr'])
        elif self.opts['optim'] == 'rmsprop':
            my_optim = keras.optimizers.RMSprop(lr = self.opts['lr'])
 
        model.compile(loss = 'binary_crossentropy', optimizer = my_optim, metrics = ['binary_accuracy'])
        #print(model.summary())
    ...
    def cv_genetic_nn(self):
        ...
        if pickMethod == "nn":
            ...
            print("above fit")
            external_mod = self.genetic_model.fit(x_train, y_train, epochs = 100, batch_size = 32, class_weight = class_weight, validation_split = 0.8, verbose=0)
            print("below fit")
            self.genetic_acc = self.genetic_model.evaluate(x_test, y_test, verbose = 0)[1]
            ...
            if plotIt:
                ...
                plt.savefig('nn_epochs' + '_'.join(self.nn_args) + '.png')
                ...
                if comparePrs:
                    ...
                    plt.ylabel('NN Probs')
                    plt.xlabel('PRS Test')
                    plt.savefig('nn_vs_prs' + '_'.join(self.nn_args) + '.png')
