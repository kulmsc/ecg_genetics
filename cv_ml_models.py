...
class Models():
    ...
    def cv_genetic_nn(self):
        ...
        control_inds = np.ceil(np.arange(all_controls.shape[0])/(all_controls.shape[0]/3))
        control_inds[0] = 1
        np.random.shuffle(control_inds)
        case_inds = np.ceil(np.arange(all_cases.shape[0])/(all_cases.shape[0]/3))
        case_inds[0] = 1
        ...
        for i in range(3):
             ...
             if pickMethod == "logreg":
                 ...
                 #the class weight method
                 class_weight = {0:sum(y_train==1)/y_train.shape[0], 1:sum(y_train==0)/y_train.shape[0]}
                 self.genetic_model.fit(x_train, y_train, epochs = 75, batch_size = 500, class_weight = class_weight)
                 #self.genetic_model.fit(x_train, y_train, epochs = 150, batch_size = 15)
                 self.genetic_acc[i] = self.genetic_model.evaluate(x_test, y_test)[1]
                 probs = self.genetic_model.predict(x_test)

                 #pdb.set_trace()
                 fpr, tpr, thresholds = metrics.roc_curve(y_test, probs[:,0])
                 self.genetic_auc[i] = metrics.auc(fpr, tpr)
                 self.genetic_pred_yes[i] = sum(y_test[np.round(probs[:,0])==1] == 1) / sum(np.round(probs[:,0])==1)
                 self.genetic_pred_no[i] = sum(y_test[np.round(probs[:,0])!=1] == 1) / sum(np.round(probs[:,0])!=1)
